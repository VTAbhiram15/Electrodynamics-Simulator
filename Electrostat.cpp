...
const int COLS = WIDTH/ARROW_SIZE;
const int ROWS = HEIGHT/ARROW_SIZE;
...
struct SimulationInit{
        ...
        point(2.f)
    {
        ...
        ch.setOrigin(12.f, 12.f);
        point.setOrigin(2.f, 2.f);
        point.setFillColor(sf::Color::White);

        FieldArrows.resize(ROWS*COLS);
    }
};
...
void updateFieldArrows(SimulationInit& sim){
    for(int i=0; i<ROWS; i++){
        for(int j=0; j<COLS; j++){
            ...
            for(const auto& c: sim.charges){
                sim.FieldArrows[index].x = j*ARROW_SIZE + (ARROW_SIZE/2);
                sim.FieldArrows[index].y = i*ARROW_SIZE + (ARROW_SIZE/2);

                float dx = sim.FieldArrows[index].x - c.x;
                float dy = sim.FieldArrows[index].y - c.y;
                float r2 = (dx*dx) + (dy*dy);
                float r = std::sqrt(r2);
                if((std::abs(r))>=CHARGE_RADIUS){
                    float E = (c.q)/r2;
                    total_Ex += E * (dx/r);
                    total_Ey += E * (dy/r);
                }
...
int main(){
    ...
    while(sim.window.isOpen()){
        ...
        while(sim.window.pollEvent(event)){
            ...
            if(event.type == sf::Event::KeyPressed && !sim.charges.empty()){
                ...
                if(event.key.code == sf::Keyboard::M && selectedIdx != -1){
                    sim.charges[selectedIdx].mass += 2.f;
                    std::cout << "mass = " << sim.charges[selectedIdx].mass << std::endl;            
                }
                if(event.key.code == sf::Keyboard::N && selectedIdx != -1){
                    sim.charges[selectedIdx].mass -= 2.f;
                    std::cout << "mass = " << sim.charges[selectedIdx].mass << std::endl;
                }
                if(event.key.code == sf::Keyboard::Delete && selectedIdx != -1){
        ...
        drawFieldArrows(arrow_len, scale_factor, sim);
        drawCharges(sim);
